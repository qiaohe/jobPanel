...
- (id)initWithJobInformation:(JobInformation*)detail
{
    ...
    if (self) {
        NSInteger param = 0;
        if ([detail.company isEqualToString:@"网易"]) {
            param = 1;
        }else if ([detail.company isEqualToString:@"腾讯"]){
            param = 2;
        }else if ([detail.company isEqualToString:@"新浪"]){
            param = 3;
        }else if ([detail.company isEqualToString:@"搜狐"]){
            param = 4;
        }else{
            param = 1;
        }
        [self getCompanyDetailWithParam:param];
    }
    return self;
}
...
- (id)init
{
    ...
    if (self) {
        ...
        [self getCompanyDetailWithParam:index];
    }
    return self;
}

- (void)getCompanyDetailWithParam:(NSInteger)param
{
    switch (param) {
        case 1:{
            _title              = @"网易";
